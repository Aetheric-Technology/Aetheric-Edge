...
mod tests {
    ...
    fn test_mqtt_topics() {
        ...
        assert_eq!(topic_builder.command_response("cmd-123"), "ae/test-device/cmd/cmd-123/response");
        
        // Test topic parsing
        let command_id = mqtt::topics::parse_command_topic("ae/test-device/cmd/install-001", "test-device");
        assert_eq!(command_id, Some("install-001".to_string()));
        
        let invalid_topic = mqtt::topics::parse_command_topic("ae/other-device/cmd/test", "test-device");
