...
impl PluginManager {
    ...
    fn ensure_directories(&self) -> Result<()> {
        ...
        std::fs::create_dir_all(&install_dir)
            .with_context(|| format!("Failed to create plugin install directory: {}", install_dir.display()))?;
            
        std::fs::create_dir_all(&temp_dir)
            .with_context(|| format!("Failed to create plugin temp directory: {}", temp_dir.display()))?;
    ...
    pub async fn install_plugin(
        ...
        self.set_plugin_status(plugin_name, PluginStatus::Installing).await?;

        match source {
            PluginSource::Url { url, checksum, checksum_type } => {
                self.install_from_url(plugin_name, url, checksum.as_deref(), checksum_type.as_deref(), config).await
            }
            PluginSource::Base64 { data, checksum, checksum_type } => {
                self.install_from_base64(plugin_name, data, checksum.as_deref(), checksum_type.as_deref(), config).await
            }
            PluginSource::AptPackage { package, version, repository } => {
                self.install_apt_package(plugin_name, package, version.as_deref(), repository.as_deref(), config).await
            }
            PluginSource::Docker { image, tag, registry } => {
                self.install_docker_plugin(plugin_name, image, tag.as_deref(), registry.as_deref(), config).await
            }
            PluginSource::Chunked { chunk_id, total_chunks, chunk_index, data, checksum } => {
                self.handle_chunked_transfer(plugin_name, chunk_id, *total_chunks, *chunk_index, data, checksum.as_deref(), config).await
            }
            PluginSource::Local { path } => {
                self.install_from_local_path(plugin_name, path, config).await
            }
        }
    }
    ...
    pub async fn update_plugin(
        ...
        self.set_plugin_status(plugin_name, PluginStatus::Updating).await?;
        ...
        // Install new version
        match self.install_plugin(plugin_name, source, plugin_config).await {
            Ok(result) => {
    ...
    pub async fn remove_plugin(&self, plugin_name: &str) -> Result<serde_json::Value> {
        ...
        self.set_plugin_status(plugin_name, PluginStatus::Uninstalling).await?;
    ...
    pub async fn start_plugin(&self, plugin_name: &str) -> Result<serde_json::Value> {
        ...
        match info.plugin_type {
            ...
            PluginType::Docker => {
                self.start_docker_plugin(plugin_name, &info).await
            }
            PluginType::AptPackage => {
                self.start_service_plugin(plugin_name, &info).await
            }
        }
    }
    ...
    pub async fn stop_plugin(&self, plugin_name: &str) -> Result<serde_json::Value> {
        ...
        match info.plugin_type {
            ...
            PluginType::Docker => {
                self.stop_docker_plugin(plugin_name).await
            }
            PluginType::AptPackage => {
                self.stop_service_plugin(plugin_name).await
            }
        }
    }
    ...
    async fn install_from_url(
        ...
        self.install_binary_content(plugin_name, &content, config).await
    }
    ...
    async fn handle_chunked_transfer(
        ...
        let transfer = transfers.entry(chunk_id.to_string()).or_insert_with(|| {
            ChunkedTransfer {
                total_chunks,
        ...
        info!("Received chunk {}/{} for plugin {}", chunk_index + 1, total_chunks, plugin_name);

        // Check if all chunks received
        if transfer.received_chunks.len() == total_chunks as usize {
            info!("All chunks received for plugin {}, assembling file", plugin_name);
            ...
            for i in 0..total_chunks {
                if let Some(chunk) = transfer.received_chunks.get(&i) {
                    ...
                    let chunk_str = String::from_utf8(chunk.clone())
                        .context("Invalid UTF-8 in chunk data")?;
            ...
            let complete_data = general_purpose::STANDARD.decode(&complete_base64)
            ...
            self.install_binary_content(plugin_name, &complete_data, plugin_config).await
        } else {
            Ok(serde_json::json!({
                "plugin_name": plugin_name,
                "chunk_id": chunk_id,
    ...
    async fn install_binary_content(
        ...
        self.set_plugin_status(plugin_name, PluginStatus::Installed).await?;
    ...
    fn verify_checksum(&self, data: &[u8], expected: &str, checksum_type: &str) -> Result<()> {
        ...
        if actual.to_lowercase() != expected.to_lowercase() {
            return Err(anyhow!("Checksum verification failed: expected {}, got {}", expected, actual));
        }
    ...
    async fn get_plugin_config(&self, plugin_name: &str) -> Result<PluginConfig> {
        let config_path = self.config.plugins.install_dir
            .join(plugin_name)
            .join("plugin.toml");
    ...
    async fn detect_plugin_status(&self, plugin_name: &str, config: &PluginConfig) -> PluginStatus {
        match config.plugin_type {
            PluginType::Docker => {
                ...
                let output = Command::new("docker")
                    .args(&["ps", "--filter", &format!("name={}", plugin_name), "--format", "{{.Names}}"])
                    .output();
    ...
    async fn backup_plugin(&self, plugin_name: &str) -> Result<()> {
        let plugin_path = self.config.plugins.install_dir.join(plugin_name);
        let backup_path = self.config.plugins.install_dir.join(format!("{}.backup", plugin_name));
    ...
    async fn copy_dir_all(&self, src: &Path, dst: &Path) -> Result<()> {
        ...
        fn copy_dir_all_recursive(src: PathBuf, dst: PathBuf) -> Pin<Box<dyn Future<Output = Result<()>> + Send>> {
            Box::pin(async move {
    ...
    async fn start_binary_plugin(&self, plugin_name: &str, info: &PluginInfo) -> Result<serde_json::Value> {
        // For binary plugins, use systemd service
        let service_name = format!("aetheric-plugin-{}", plugin_name);
        
        // Ensure systemd service exists
        self.create_systemd_service_for_plugin(plugin_name, info).await?;
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(anyhow!("Failed to start plugin service {}: {}", service_name, stderr));
        }
    ...
    async fn stop_binary_plugin(&self, plugin_name: &str, _info: &PluginInfo) -> Result<serde_json::Value> {
        let service_name = format!("aetheric-plugin-{}", plugin_name);
    ...
    async fn create_systemd_service_for_plugin(&self, plugin_name: &str, info: &PluginInfo) -> Result<()> {
        let service_name = format!("aetheric-plugin-{}", plugin_name);
...
"#, 
    ...
    async fn start_docker_plugin(&self, plugin_name: &str, info: &PluginInfo) -> Result<serde_json::Value> {
        ...
        info!("Started Docker plugin {} with container ID {}", plugin_name, container_id);
    ...
    async fn start_service_plugin(&self, plugin_name: &str, _info: &PluginInfo) -> Result<serde_json::Value> {
        let output = Command::new("sudo")
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(anyhow!("Failed to start service {}: {}", plugin_name, stderr));
        }
    ...
    pub async fn enable_plugin(&self, plugin_name: &str) -> Result<serde_json::Value> {
        ...
        if let Ok(mut plugin_info) = self.load_plugin_info(&config_path).await {
            ...
            if matches!(plugin_info.status, PluginStatus::Running | PluginStatus::Stopped) {
                let _ = self.start_plugin(plugin_name).await;
            }
        }
    ...
    pub async fn set_plugin_maintenance(&self, plugin_name: &str, maintenance_mode: bool, reason: Option<String>) -> Result<serde_json::Value> {
        info!("Setting maintenance mode for plugin {}: {} - {}", plugin_name, maintenance_mode, reason.as_deref().unwrap_or("No reason"));
    ...
    pub async fn check_plugin_health(&self, plugin_name: &str) -> Result<PluginHealthStatus> {
        ...
        let plugin_info = self.load_plugin_info(&config_path).await
        ...
        if is_unhealthy {
            ...
            if plugin_info.health_check_failures >= MAX_HEALTH_FAILURES {
                if plugin_info.restart_count < MAX_RESTART_ATTEMPTS {
                    info!("Restarting unhealthy plugin: {} (attempt {}/{})", plugin_name, plugin_info.restart_count + 1, MAX_RESTART_ATTEMPTS);
                    ...
                    warn!("Plugin {} exceeded maximum restart attempts, marking as failed", plugin_name);
    ...
    async fn is_plugin_unhealthy(&self, plugin_info: &PluginInfo) -> Result<bool> {
        match plugin_info.plugin_type {
            ...
            PluginType::Docker => {
                self.is_docker_container_unhealthy(plugin_info).await
            }
            PluginType::AptPackage => {
    ...
    async fn get_plugin_resource_usage(&self, plugin_info: &PluginInfo) -> (f32, u64) {
        match plugin_info.plugin_type {
            ...
            PluginType::Docker => {
                self.get_docker_container_resources(plugin_info).await
            }
            PluginType::AptPackage => {
                self.get_systemd_service_resources(plugin_info).await
            }
    ...
    async fn save_plugin_info(&self, config_path: &std::path::Path, plugin_info: &PluginInfo) -> Result<()> {
