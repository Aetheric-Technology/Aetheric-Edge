...
fn validate_qos(qos: u8) -> Result<QoS> {
    match qos {
        ...
        _ => Err(anyhow::anyhow!("Invalid QoS level: {}. Must be 0, 1, or 2", qos)),
    }
}
...
mod tests {
    ...
    fn test_topic_validation_valid_topics() {
        ...
        for topic in valid_topics {
            assert!(validate_topic(topic).is_ok(), "Topic should be valid: {}", topic);
        }
    }
    ...
    fn test_json_validation_valid() {
        ...
        for json in valid_json_examples {
            assert!(validate_json_message(json).is_ok(), "Should be valid JSON: {}", json);
        }
    }
    ...
    fn test_json_validation_invalid() {
        ...
        for json in invalid_json_examples {
            assert!(validate_json_message(json).is_err(), "Should be invalid JSON: {}", json);
        }
    }
    ...
    fn test_extreme_message_sizes() {
        ...
        for (size, limit, should_pass) in test_cases {
            ...
            if should_pass {
                assert!(result.is_ok(), "Size {} should pass with limit {}", size, limit);
            } else {
                assert!(result.is_err(), "Size {} should fail with limit {}", size, limit);
            }
        }
    }
    ...
    fn test_unicode_in_topics_and_messages() {
        ...
        for topic in unicode_topics {
            ...
            assert!(validate_topic(topic).is_ok(), "Unicode topic should be valid: {}", topic);
        }
    ...
    fn test_special_characters_in_topics() {
        ...
        for topic in special_topics {
            ...
            assert!(validate_topic(topic).is_ok(), "Topic with special chars should be valid: {}", topic);
        }
    }
    ...
    fn test_client_id_edge_cases() {
        ...
        for (client_id, should_pass) in test_cases {
            ...
            if should_pass {
                ...
                assert!(result.is_err(), "Client ID '{}' should be invalid", client_id);
            }
        }
    }
