...
async fn test_ssh_session_id_validation() {
    ...
    for session_id in session_ids {
        ...
        if session_id.trim().is_empty() {
            ...
            assert!(
                response.message.contains("Invalid session ID")
                    || response.message.contains("Session ID cannot be empty")
            );
        }
    }
}
...
async fn test_ssh_concurrent_session_limits() {
    ...
    let connected_count = responses
        .iter()
        .filter(|r| matches!(r.status, SshSessionStatus::Connected))
        .count();

    let limit_exceeded_count = responses
        .iter()
...
async fn test_ssh_data_direction_validation() {
    ...
    let directions = vec![SshDataDirection::Up, SshDataDirection::Down];
...
async fn test_ssh_command_serialization_security() {
    ...
    for command in test_commands {
        ...
        match (&command, &deserialized) {
            ...
            (
                SshCommand::Disconnect { session_id: id1 },
                SshCommand::Disconnect { session_id: id2 },
            ) => {
                assert_eq!(id1, id2);
            }
            (
            ...
            (
                SshCommand::Heartbeat { session_id: id1 },
                SshCommand::Heartbeat { session_id: id2 },
            ) => {
                assert_eq!(id1, id2);
            }
...
async fn test_ssh_disabled_security() {
    ...
    assert_eq!(response.status, SshSessionStatus::Failed);
    assert!(
        response.message.contains("SSH is disabled") || response.message.contains("not enabled")
    );
}
