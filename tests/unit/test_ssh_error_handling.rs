...
async fn test_invalid_base64_data() {
    ...
    for (i, invalid_data) in invalid_base64_data.iter().enumerate() {
        ...
        assert!(
            ...
            "Unexpected error message for case {}: {}",
            i,
            response.message
        );
    }
    ...
    for (session_id, should_be_valid) in test_cases {
        ...
        if !should_be_valid {
            assert_eq!(
                response.status,
                SshSessionStatus::Failed,
                "Session ID length {} should be rejected",
                session_id.len()
            );
        }
        ...
        match response.status {
            SshSessionStatus::Failed => {
                ...
                assert!(
                    ...
                    "Error message should not be empty for malformed host: {}",
                    host
                );
            }
...
async fn test_session_limit_boundary() {
    ...
    for i in 0..3 {
        // max_sessions = 2, so 3rd should fail
        let connect_command = SshCommand::Connect {
    ...
    let _limit_failed_count = responses
        .iter()
