...
async fn test_high_frequency_commands() {
    ...
    for i in 0..command_count {
        ...
        let command = match i % 4 {
            ...
            2 => SshCommand::Data {
                ...
                data: base64::Engine::encode(&base64::engine::general_purpose::STANDARD, b"stress test data"),
    ...
    println!("Processed {} commands in {:?} ({:.2} commands/second)", 
             command_count, elapsed, commands_per_second);
    
    // Should handle at least most commands successfully
    assert!(successful_commands > command_count * 90 / 100, "Too many command failures");
    
    // Should process commands reasonably quickly (at least 100 commands/second)
    assert!(commands_per_second > 100.0, "Command processing too slow: {:.2} commands/second", commands_per_second);
}
...
async fn test_concurrent_session_creation() {
    ...
    println!("Created {} successful sessions out of {} attempts in {:?} (avg: {:?})", 
    ...
    assert!(successful_connections > 0, "No sessions created successfully");
    
    // Average response time should be reasonable (under 100ms)
    assert!(avg_duration < Duration::from_millis(100), "Session creation too slow: {:?}", avg_duration);
}
...
async fn test_data_throughput_stress() {
    ...
    let connect_response = ssh_manager.handle_ssh_command(connect_command).await.unwrap();
    ...
    if matches!(connect_response.status, SshSessionStatus::Connected) {
        ...
        for i in 0..data_packets {
            let direction = if i % 2 == 0 { SshDataDirection::Up } else { SshDataDirection::Down };
        ...
        let throughput_mbps = (successful_packets * packet_size) as f64 / (1024.0 * 1024.0) / elapsed.as_secs_f64();
        
        println!("Sent {} packets ({} KB each) in {:?} ({:.2} packets/sec, {:.2} MB/s)", 
                 successful_packets, packet_size / 1024, elapsed, packets_per_second, throughput_mbps);
        ...
        assert!(successful_packets > data_packets * 90 / 100, "Too many packet failures");
        
        // Should achieve reasonable throughput (at least 50 packets/second)
        assert!(packets_per_second > 50.0, "Data throughput too slow: {:.2} packets/second", packets_per_second);
    }
}
...
async fn test_session_churn_stress() {
    ...
    for cycle in 0..cycles {
        ...
        for session_id in &cycle_sessions {
            let data_command = SshCommand::Data {
                session_id: session_id.clone(),
                data: base64::Engine::encode(&base64::engine::general_purpose::STANDARD, b"churn test"),
                direction: SshDataDirection::Up,
            };
        ...
        assert_eq!(session_count, 0, "Sessions not cleaned up after cycle {}", cycle);
    }
    
    let elapsed = start_time.elapsed();
    let cycles_per_second = cycles as f64 / elapsed.as_secs_f64();
    
    println!("Completed {} churn cycles ({} sessions each) in {:?} ({:.2} cycles/sec)", 
             cycles, sessions_per_cycle, elapsed, cycles_per_second);
    println!("Successful connections: {}, disconnections: {}", 
             total_successful_connections, total_successful_disconnections);
    
    // Should complete cycles at reasonable speed
    assert!(cycles_per_second > 1.0, "Session churn too slow: {:.2} cycles/second", cycles_per_second);
    
    // Should have some successful operations
    assert!(total_successful_connections > 0, "No successful connections");
}
    ...
    for batch in 0..(total_sessions / batch_size) {
        ...
        for session_id in &batch_sessions {
            for _ in 0..5 {
                let data_command = SshCommand::Data {
                    session_id: session_id.clone(),
                    data: base64::Engine::encode(&base64::engine::general_purpose::STANDARD, &vec![0u8; 1024]),
                    direction: SshDataDirection::Up,
                };
        ...
        assert_eq!(current_session_count, 0, "Memory leak detected - sessions not cleaned up in batch {}", batch);
    }
    
    // Final verification
    let final_session_count = ssh_manager.get_session_count().await;
    assert_eq!(final_session_count, 0, "Memory leak detected - final session count not zero");
}
...
async fn test_error_handling_under_stress() {
    ...
    println!("Stress test completed: {} total operations, {} successful, {} failed", 
    ...
    assert!(failed_operations > total_operations / 2, "Expected more graceful failures");
    ...
    assert!(final_response.is_ok(), "System should remain responsive after stress test");
}
...
async fn test_long_running_session_stress() {
    ...
    if matches!(connect_response.status, SshSessionStatus::Connected) {
        ...
        for i in 0..operations_count {
            ...
            let heartbeat_response = ssh_manager.handle_ssh_command(heartbeat_command).await.unwrap();
            assert_eq!(heartbeat_response.status, SshSessionStatus::Connected, "Heartbeat {} failed", i);
            ...
            let data_command = SshCommand::Data {
                ...
                direction: if i % 2 == 0 { SshDataDirection::Up } else { SshDataDirection::Down },
            };
        ...
        println!("Long-running session: {} operations in {:?} ({:.2} ops/sec)", 
        ...
        assert!(operations_per_second > 10.0, "Long-running session performance degraded: {:.2} ops/sec", operations_per_second);
    }
}