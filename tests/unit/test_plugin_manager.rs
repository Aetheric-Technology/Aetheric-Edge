use aetheric_edge::agent::plugin_manager::{
    PluginManager, PluginSource, PluginConfig, PluginType
};
...
async fn test_install_plugin_from_local() {
    ...
    let result = plugin_manager.install_plugin("test-local-plugin", &source, plugin_config).await.unwrap();
...
async fn test_install_plugin_from_base64() {
    ...
    let result = plugin_manager.install_plugin("test-base64-plugin", &source, plugin_config).await.unwrap();
    
    assert_eq!(result["plugin_name"], "test-base64-plugin");
    assert_eq!(result["status"], "installed");
    
    // Verify the file was created and has correct content
    let installed_path = config.plugins.install_dir.join("test-base64-plugin").join("plugin.sh");
...
async fn test_install_plugin_invalid_base64() {
    ...
    let result = plugin_manager.install_plugin("test-invalid-plugin", &source, plugin_config).await;
...
async fn test_install_plugin_nonexistent_local_file() {
    ...
    let result = plugin_manager.install_plugin("test-nonexistent-plugin", &source, plugin_config).await;
    assert!(result.is_err());
}
...
async fn test_remove_plugin() {
    ...
    let install_result = plugin_manager.install_plugin("test-remove-plugin", &source, plugin_config).await.unwrap();
    assert_eq!(install_result["status"], "installed");
    
    // Now remove the plugin
    let remove_result = plugin_manager.remove_plugin("test-remove-plugin").await.unwrap();
...
async fn test_remove_nonexistent_plugin() {
    ...
    let result = plugin_manager.remove_plugin("nonexistent-plugin").await.unwrap();
...
async fn test_chunked_transfer() {
    ...
    let decoded_data = base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &base64_data).unwrap();
    ...
    let result1 = plugin_manager.install_plugin("test-chunked-plugin", &source1, plugin_config.clone()).await.unwrap();
    ...
    let result2 = plugin_manager.install_plugin("test-chunked-plugin", &source2, plugin_config).await.unwrap();
    ...
    let installed_path = config.plugins.install_dir.join("test-chunked-plugin").join("plugin.bin");
    assert!(installed_path.exists());
    let content = tokio::fs::read(&installed_path).await.unwrap();
...
async fn test_chunked_transfer_out_of_order() {
    ...
    let result2 = plugin_manager.install_plugin("test-ooo-plugin", &source2, plugin_config.clone()).await.unwrap();
    ...
    let result1 = plugin_manager.install_plugin("test-ooo-plugin", &source1, plugin_config).await.unwrap();
    ...
    let installed_path = config.plugins.install_dir.join("test-ooo-plugin").join("plugin.bin");
    assert!(installed_path.exists());
    let content = tokio::fs::read(&installed_path).await.unwrap();
    assert_eq!(content, test_data);
}
...
async fn test_plugin_config_serialization() {
    ...
    let config = PluginConfig {
        ...
        command_args: vec!["--verbose".to_string(), "--config=/app/config.json".to_string()],
    };
...
async fn test_list_plugins_with_installed_plugins() {
    ...
    plugin_manager.install_plugin("list-test-plugin", &source, plugin_config).await.unwrap();
    ...
    let our_plugin = plugins.iter().find(|p| p["plugin_name"] == "list-test-plugin");
