use aetheric_edge::agent::ssh_tunnel::{SshTunnelManager, SshCommand, SshDataDirection, SshSessionStatus, SshSession};
...
async fn test_session_lifecycle_complete() {
    ...
    if matches!(connect_response.status, SshSessionStatus::Connected) {
        ...
        let disconnect_response = ssh_manager.handle_ssh_command(disconnect_command).await.unwrap();
...
async fn test_multiple_sessions_management() {
    ...
    if !successful_sessions.is_empty() {
        ...
        for session_id in &successful_sessions {
            ...
            let _response = ssh_manager.handle_ssh_command(disconnect_command).await.unwrap();
        }
...
async fn test_session_data_handling() {
    ...
    if matches!(connect_response.status, SshSessionStatus::Connected) {
        ...
        for (i, data) in test_data.iter().enumerate() {
            ...
            assert_eq!(up_response.session_id, session_id, "Failed on data test {}", i);
            ...
            assert_eq!(down_response.session_id, session_id, "Failed on data test {}", i);
        }
    ...
    for i in 0..session_count {
        ...
        if matches!(response.status, SshSessionStatus::Connected) {
            if let Some(local_port) = response.local_port {
                ...
                assert!(local_port >= 10000 && local_port < 11000, "Port {} out of range", local_port);
                
                // Port should be unique
                assert!(!allocated_ports.contains(&local_port), "Duplicate port {} allocated", local_port);
...
async fn test_session_metadata() {
    ...
    if matches!(connect_response.status, SshSessionStatus::Connected) {
        ...
        assert!(current_time - session.created_at < 5, "Created time seems wrong");
        
        // Last activity should be recent
        assert!(current_time - session.last_activity < 5, "Last activity time seems wrong");
