...
async fn test_mqtt_client_topic_generation() {
    ...
    assert_eq!(topic_builder.command_response("cmd-123"), "ae/test-device-001/cmd/cmd-123/response");
}
...
async fn test_ssh_data_command_serialization() {
    ...
    match deserialized {
        CommandType::SshData { session_id, data, direction } => {
            assert_eq!(session_id, "ssh-session-001");
            let decoded_data = base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &data).unwrap();
...
async fn test_event_message_serialization() {
    ...
    metadata.insert("component".to_string(), serde_json::Value::String("ssh_tunnel".to_string()));
    metadata.insert("session_count".to_string(), serde_json::Value::Number(serde_json::Number::from(3)));
...
async fn test_all_command_types_serialization() {
    let commands = vec![
        ...
        CommandType::Remove { plugin_name: "test".to_string() },
        CommandType::Start { plugin_name: "test".to_string() },
        CommandType::Stop { plugin_name: "test".to_string() },
        CommandType::Restart { plugin_name: "test".to_string() },
        CommandType::Status { plugin_name: Some("test".to_string()) },
        CommandType::Status { plugin_name: None },
        CommandType::SshDisconnect { session_id: "session-1".to_string() },
        CommandType::SshHeartbeat { session_id: "session-1".to_string() },
    ...
    for command in commands {
        ...
        match (&command, &deserialized) {
            ...
            (CommandType::Restart { plugin_name: n1 }, CommandType::Restart { plugin_name: n2 }) => {
                assert_eq!(n1, n2);
            },
            (CommandType::Status { plugin_name: n1 }, CommandType::Status { plugin_name: n2 }) => {
                assert_eq!(n1, n2);
            },
            (CommandType::SshDisconnect { session_id: s1 }, CommandType::SshDisconnect { session_id: s2 }) => {
                assert_eq!(s1, s2);
            },
            (CommandType::SshHeartbeat { session_id: s1 }, CommandType::SshHeartbeat { session_id: s2 }) => {
                assert_eq!(s1, s2);
            },
            (CommandType::OtaUpdate { version: v1, url: u1, checksum: c1 }, 
             CommandType::OtaUpdate { version: v2, url: u2, checksum: c2 }) => {
                assert_eq!(v1, v2);
                assert_eq!(u1, u2);
                assert_eq!(c1, c2);
            },
...
async fn test_topic_builder() {
    ...
    assert_eq!(builder.command_response("cmd-456"), "ae/test-device-123/cmd/cmd-456/response");
    ...
    assert_eq!(builder.ssh_tunnel("session-789"), "ae/test-device-123/ssh/session-789");
}