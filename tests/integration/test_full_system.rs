...
async fn setup_test_agent(temp_dir: &TempDir) -> (Agent, Arc<MqttClient>, mpsc::UnboundedSender<CommandMessage>) {
...
async fn test_plugin_install_complete_flow() {
    ...
    tokio::fs::write(&test_plugin_path, b"#!/bin/bash\necho 'Integration test plugin'\n").await.unwrap();
...
async fn test_configuration_validation() {
    ...
    assert_eq!(config.plugins.docker_enabled, false);
    assert!(config.plugins.install_dir.to_string_lossy().contains("plugins"));
    assert!(config.plugins.install_dir.to_string_lossy().contains("plugins"));
}
