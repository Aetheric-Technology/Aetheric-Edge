use aetheric_edge::agent::{CommandHandler, HealthMonitor};
...
async fn extreme_mqtt_message_flooding() {
    ...
    assert!(
        success_count > message_count * 80 / 100,
        "Should handle at least 80% of messages"
    );
}
...
async fn extreme_concurrent_command_processing() {
    ...
    assert!(
        success_count >= 90,
        "Should process at least 90% of commands successfully"
    );
}
...
async fn extreme_health_monitoring_stress() {
    ...
    let monitor_handle = tokio::spawn(async move {
        let _ = timeout(Duration::from_secs(5), health_monitor.run(mqtt_client)).await;
    });

    // Simulate CPU and memory stress
    let stress_handles: Vec<_> = (0..10)
        .map(|_| {
            tokio::spawn(async {
...
async fn extreme_ssh_session_limits() {
    ...
    for i in 0..10 {
        let result = ssh_manager
            .handle_ssh_command(aetheric_edge::agent::ssh_tunnel::SshCommand::Connect {
    ...
    let connected_count = session_results
        .iter()
        .filter(|r| {
            matches!(
                r.status,
                aetheric_edge::agent::ssh_tunnel::SshSessionStatus::Connected
            )
        })
        .count();
    ...
    for i in 0..50 {
        ...
        if matches!(
            ...
            let _ = ssh_manager
                .handle_ssh_command(aetheric_edge::agent::ssh_tunnel::SshCommand::Disconnect {
                    session_id: format!("rapid-{}", i),
                })
                .await;
        }
...
async fn extreme_chunked_transfer_stress() {
    ...
    assert!(
        ...
        "Chunked transfer should complete successfully"
    );
            ...
            let _ = pm
                .install_plugin(
                    ...
                    aetheric_edge::agent::plugin_manager::PluginConfig {
                        ...
                        auto_start: false,
                        environment: (0..1000)
                            .map(|j| (format!("KEY_{}", j), "VALUE".repeat(100)))
                            .collect(),
